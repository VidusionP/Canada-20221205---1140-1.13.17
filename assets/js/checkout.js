                    ...
                    if (item["Total On Hand"] == item["Available Quantity"]) {
                        if (Number(item["Quantity Canada"])>0) {
                            if (Number(item["Quantity Canada"])-buff>0) {
                                content+=`Immediately: quantity <strong>${Math.min(item["Quantity Canada"]-buff, qty)}</strong><br/>`;
                        ...
                        if (Number(item["Quantity USA"])>0 && qty>0) {
                            if (Number(item["Quantity USA"])-buff>0) {
                                content+=`2-4 days: quantity <strong>${Math.min(item["Quantity USA"]-buff, qty)}</strong><br/>`;
        ...
        if ($(".checkout-step--shipping").find(".postal-code").length>0) {
            ...
            if (dlvrZip.includes(tzip.toUpperCase())==false || tcountry.toLowerCase().includes("canada")==false) {
                ...
                $(".checkout-step--payment")
                    ...
                    .find("li.form-checklist-item")
                    .filter(function () {
                        return $(this).find(".paymentProviderHeader-name").eq(0).text().toLowerCase().includes("cash on delivery");
                    })
                    .css("display", "none");
            }
        }                
