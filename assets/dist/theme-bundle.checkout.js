 ...
 */function updateSI(element){if(element.checked){showLoading('Updating Shipping Insurance');var totalAmount=cartItems.baseAmount;var _insurance=Math.floor(totalAmount*0.01);var url='/cart.php?action=add&sku=SKUSI-'+(_insurance-9);__WEBPACK_IMPORTED_MODULE_0_jquery___default.a.get(url,function(data){if(isPopup){location.href='/checkout?popup=true'}else{location.href='/checkout'}})}else{__WEBPACK_IMPORTED_MODULE_1_sweetalert2___default()({text:'You will NOT be covered for your order in case it is lost, damaged or stolen. Are you sure to remove this option?',type:'warning',showCancelButton:true}).then(function(){// remove item from cart
 ...
 */function cartCheckSI(){var totalAmount=cartItems.baseAmount;shippingItems=findShippingInsurance(cartItems.lineItems.physicalItems);if(shippingItems){if(shippingItems.length>1){updateLoadingContent('Updating Shipping Insurance');var salePrice=shippingItems.reduce(function(a,b){return a+b.salePrice},0);var totalWithoutShipping=totalAmount-salePrice;cartRemoveSI(0,shippingItems,function(){if(totalWithoutShipping>=1000){var _insurance2=Math.floor(totalWithoutShipping*0.01);var url='/cart.php?action=add&sku=SKUSI-'+(_insurance2-9);__WEBPACK_IMPORTED_MODULE_0_jquery___default.a.get(url,function(data){if(isPopup){location.href='/checkout?popup=true'}else{location.href='/checkout'}})}else{if(isPopup){location.href='/checkout?popup=true'}else{location.href='/checkout'}}})}else{var _salePrice=shippingItems[0].salePrice;var _totalWithoutShipping=totalAmount-_salePrice;var _insurance3=Math.floor(_totalWithoutShipping*0.01);if(_insurance3!=Math.ceil(_salePrice)){updateLoadingContent('Updating Shipping Insurance');cartRemoveSI(0,shippingItems,function(){if(_totalWithoutShipping>=1000){var url='/cart.php?action=add&sku=SKUSI-'+(_insurance3-9);__WEBPACK_IMPORTED_MODULE_0_jquery___default.a.get(url,function(data){if(isPopup){location.href='/checkout?popup=true'}else{location.href='/checkout'}})}else{if(isPopup){location.href='/checkout?popup=true'}else{location.href='/checkout'}}})}else{hideLoading();if(_totalWithoutShipping>=1000&&_totalWithoutShipping<2000){embeddedSI(true)}if(teamdeskItems){cartSetDelivery()}else{cartGetDelivery()}}}}else{if(totalAmount>=1000&&totalAmount<2000){var _sioParam=getParameterByName('sio');if(_sioParam){__WEBPACK_IMPORTED_MODULE_0_jquery___default()('#cart-edit-link').attr('href','/cart.php?sio=true');hideLoading();if(teamdeskItems){cartSetDelivery()}else{cartGetDelivery()}embeddedSI(false)}else{var _insurance4=Math.floor(totalAmount*0.01);var url='/cart.php?action=add&sku=SKUSI-'+(_insurance4-9);updateLoadingContent('Updating Shipping Insurance');__WEBPACK_IMPORTED_MODULE_0_jquery___default.a.get(url,function(data){if(isPopup){location.href='/checkout?popup=true'}else{location.href='/checkout'}})}}else if(totalAmount>=2000){if(insurance<=106){var _url='/cart.php?action=add&sku=SKUSI-'+(insurance-9);updateLoadingContent('Updating Shipping Insurance');__WEBPACK_IMPORTED_MODULE_0_jquery___default.a.get(_url,function(data){if(isPopup){location.href='/checkout?popup=true'}else{location.href='/checkout'}})}else{hideLoading();if(teamdeskItems){cartSetDelivery()}else{cartGetDelivery()}}}else{hideLoading();if(teamdeskItems){cartSetDelivery()}else{cartGetDelivery()}}}}/**
